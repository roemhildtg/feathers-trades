...
export const tasks = (app: Application) => {
  ...
  app.service('tasks').hooks({
    ...
    after: {
      ...
      create: [
        (context) => {
          const queue = app.get('queue') as PgBoss;
          const document = (context.result as Tasks).document_name;
          logger.info('dispatching job: ' +  document)
          queue.send('ocr-document', {documentName: document })
        }
      ]
    },
    error: {
